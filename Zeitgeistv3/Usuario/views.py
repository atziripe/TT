...
def login(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            response = requests.post('http://54.200.104.36/v3/token/', data=json.dumps(
            ...
            if (response.ok):
                ...
                typeuser = requests.get('http://54.200.104.36/v1/'+str(tipo)+'user/' + str(
            ...
            if fregC.is_valid() and re.match('^[(a-z0-9\_\-\.)]+@[(a-z0-9\_\-\.)]+\.[(a-z)]{2,15}$', fregC.cleaned_data['correo'].lower()):
                ...
                if pwd == pwd2:
                    if ValidarContrasena(pwd):
                        ...
                        response = requests.post('http://54.200.104.36/v1/createuser/', data=json.dumps(
                            payload), headers={'content-type': 'application/json'})
                        print(response.json())
                        if(response.ok):
                            payload = {
                            ...
                            registerC = requests.post('http://54.200.104.36/v1/createcare/', data=json.dumps(
                            payload), headers={'content-type': 'application/json'})
                            if(registerC.ok):
                        ...
                        if pwd == pwd2:
                            if ValidarContrasena(pwd):
                                ...
                                if(response.ok):
                                    ...
                                    registerE = requests.post('http://54.200.104.36/v1/createspecialist/', data=json.dumps(
                                    payload), headers={'content-type': 'application/json'})
                                    if(registerE.ok):
            ...
            if fregP.is_valid() and re.match('^[(a-z0-9\_\-\.)]+@[(a-z0-9\_\-\.)]+\.[(a-z)]{2,15}$', fregP.cleaned_data['correo'].lower()):
                ...
                if pwd == pwd2:
                    ...
                    if(response.ok):
                        ...
                        registerC = requests.post('http://54.200.104.36/v1/createpacient/', data=json.dumps(payloadP), headers={'content-type': 'application/json'})
                        if(registerC.ok):
                ...
                if pwd == pwd2:
                    if ValidarContrasena(pwd):
                        ...
                        if(response.ok):
                            ...
                            registerA = requests.post('http://54.200.104.36/v1/createadmin/', data=json.dumps(
                            payload), headers={'content-type': 'application/json', 'Authorization': 'Bearer '+ token})
                            if(registerA.ok):
        ...
        try: 
            ...
            if pwd_n == pwd_n2:
                if ValidarContrasena(pwd_n) or tipo == "Paciente": #El paciente no tiene restriccion en la contraseña
                    ...
                    response = requests.put('http://54.200.104.36/v3/cambiarpwd/'+str(iduser)+'/', data=json.dumps(
                        payload), headers={'content-type': 'application/json', 'Authorization': 'Bearer '+ token})
                    if(response.ok):
